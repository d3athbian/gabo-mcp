...
describe('detectCredentials', () => {
  ...
  describe('should detect specific service patterns', () => {
    ...
    it('detects MongoDB+srv connection string', () => {
      const result = detectCredentials('mongodb+srv://user:pass@cluster.mongodb.net/test', 'test');
      expect(result.detected).toBe(true);
    });
    ...
    it('detects MySQL connection string', () => {
      const result = detectCredentials('mysql://user:pass@localhost:3306/testdb', 'test');
      expect(result.detected).toBe(true);
    });
  });
  ...
  describe('should not detect when no credentials present', () => {
    it('returns clean for normal text', () => {
      const result = detectCredentials('This is a normal code snippet without secrets', 'test');
  ...
  describe('should search in title as well', () => {
    it('detects credential in title', () => {
      const result = detectCredentials('Normal content', 'API configuration: secret=mykey');
      expect(result.detected).toBe(true);
    });
  });
