...
export function isLikelyFalsePositive(match: string, context: string): boolean {
  ...
  if (/^(0x[a-f0-9]+|[a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12})$/i.test(match))
    return true;

  if (/^[a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12}$/i.test(match)) return true;

  if (/^\d{4}-\d{2}-\d{2}$/.test(match)) return true;

  if (lowerContext.includes('http') && (lowerContext.includes('/') || lowerContext.includes('.'))) {
    if (
