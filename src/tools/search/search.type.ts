...
export const SearchSchema = z.object({
  ...
  mode: z
    ...
    .default("hybrid")
    .describe(
      "Search mode: text (keywords), semantic (vectors), or hybrid (both)",
    ),
  query_vector: z
    .array(z.number())
    .optional()
    .describe(
      "Vector embedding for semantic search (required for semantic/hybrid mode)",
    ),
  include_pitfalls: z
    .boolean()
    .default(false)
  ...
  include_patterns: z
    .boolean()
    .default(false)
    .describe("Include PATTERN type in results"),
  limit: z
    .number()
    .int()
    .positive()
